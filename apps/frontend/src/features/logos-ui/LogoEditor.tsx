...
function LogoEditor(data: { selectLogo: number | undefined; imgs: Array<{ icon: string; name: string }> }) {
		...
		<>
			{data.selectLogo == undefined ? (
				...
				<div className={classes.innerBox}>
					...
					<div>
						{!edit ? (
							<Text onClick={() => setEdit(true)}>{data.imgs[data.selectLogo].name}</Text>
						) : (
							<Input width="100%" variant="unstyled" defaultValue={data.imgs[data.selectLogo].name} />
						)}
					</div>
