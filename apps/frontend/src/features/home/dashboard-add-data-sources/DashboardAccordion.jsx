...
export function DashboardAccordion({sourcesTypes, sourcesInfoData, overrallWidth}) {
    ...
    return sourcesTypes.map((e, i) => {
      return (
        <Accordion.Item value={e.label} key={i}>
          <Accordion.Control icon={<img src={e.img} width={overrallWidth ? 30 : 50}/>}>{e.label}</Accordion.Control>
          <Accordion.Panel>
  ...
  return (
    <Accordion 
      ...
      className={overrallWidth ? null : classes.accordionContainer}
      width={overrallWidth}
      variant="filled"
