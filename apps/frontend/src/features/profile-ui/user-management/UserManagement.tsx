...
function UserManagement() {
	...
	const pendingTableHeads = ["Email Address", "Invitation Send On", ""];

	const userTableHeads = ["User", "Email Address", "Last Seen", ""];
	...
	const PendingTableHeads = pendingTableHeads.map((head) => <th key={head}>{head}</th>);

	const UserTableHeads = userTableHeads.map((head) => <th key={head}>{head}</th>);

	return (
		<div>
			{/* Search& share */}
			<Group position="apart">
				<Input icon={<img src={SearchIcon} width={20} />} variant="filled" placeholder="Your email" />
			...
			<div onClick={() => setOpened((o) => !o)} className={classes.collapseLink}>
				<Link
					label={`Pending Requests (${
						PendingElements.length < 10 ? `0${PendingElements.length}` : PendingElements.length
					})`}
					underline={true}
				/>
			</div>
			...
			{/* Collapse Body */}
			<Collapse in={opened}>
				<ScrollArea style={{ height: "200px" }} offsetScrollbars scrollbarSize={6} scrollHideDelay={1500}>
					<CustomTable rows={PendingTableBody} Heads={PendingTableHeads} />
				</ScrollArea>
			</Collapse>
			...
			>
				<CustomTable rows={UserTableBody} Heads={UserTableHeads} />
			</ScrollArea>
		</div>
	);
}
